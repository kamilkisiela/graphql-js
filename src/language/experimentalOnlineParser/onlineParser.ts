...
interface OnlineParserConfig {
  tabSize: number;
}

interface OnlineParserConfigOption {
...
export class OnlineParser {
  ...
  _parsePeekConstraint(rule: PeekOnlineParserRule): Token {
    ...
    while (!rule.matched && rule.index < rule.peek.length - 1) {
      ...
      if (typeof ifCondition === 'string') {
        ifCondition = GraphQLGrammar[ifCondition] as string;
      }

      let token = this._lookAhead();
      if (
        ifCondition &&
        this._matchToken(token, ifCondition as GraphQLGrammarTokenConstraint)
      ) {
        rule.matched = true;
        rule.expanded = true;
  ...
  _butNot(
    ...
    if (rule.butNot) {
      ...
      return !this._matchToken(
        token,
        rule.butNot as GraphQLGrammarTokenConstraint,
      );
    }
  ...
  _popMatchedRule(token?: Maybe<Token>) {
    ...
    if (token && rule.kind === RuleKind.TOKEN_CONSTRAINT) {
      const constraint: TokenOnlineParserRule = rule;
  ...
  _getRuleKind(rule: GraphQLGrammarRule | OnlineParserRule): string {
    ...
    if ((rule as ConstraintsSetOnlineParserRule).constraintsSet === true) {
