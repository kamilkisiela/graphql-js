...
type ElementOf<T> = T extends ReadonlyArray<infer R> ? R : never;
type MaybeOf<T> = T extends Maybe<infer R> ? R : never;
type NodeOf<
  T extends {
    astNode: Maybe<any>;
    extensionASTNodes: Maybe<ReadonlyArray<any>>;
  }
> = MaybeOf<T['astNode']> | ElementOf<MaybeOf<T['extensionASTNodes']>>;
...
>(obj: T): ReadonlyArray<NodeOf<T>> {
  const { astNode, extensionASTNodes } = obj;
...
>(
  object: O,
  getter: (_: NodeOf<O>) => Maybe<L | ReadonlyArray<L>>,
): ReadonlyArray<L> {
  let subNodes = [];
...
function getAllImplementsInterfaceNodes(
  ...
  return getAllSubNodes(
    type,
    (typeNode) => 'interfaces' in typeNode && typeNode.interfaces,
...
function getUnionMemberTypeNodes(
  ...
  return getAllSubNodes(
    union,
    (unionNode) => 'types' in unionNode && unionNode.types,
